    ...
    <div className="h-screen w-screen flex overflow-hidden font-sans">
      ...
      >
        ...
        <div className="relative z-10 mb-8">
          <button
            onClick={() => navigate(-1)}
            className="bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white px-4 py-2 rounded-lg transition-all duration-200 flex items-center gap-2 mb-4 shadow-lg hover:shadow-xl"
          >
              ...
              todayMeetings
                ...
                .map((r, i) => (
                  <div
                    key={r.id || i}
                    onClick={() => {
                      // Reload halaman dengan ID baru
                      window.location.href = `/rapat/detail/${r.id}`;
                    }}
                    className="p-3 bg-white/10 rounded-lg hover:bg-white/20 transition-all mb-3 cursor-pointer"
                  >
